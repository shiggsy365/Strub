...
/**
 * Fragment for the Library page.
 * Uses Home-style layout with two rows: Movies and Series.
...
class LibraryFragment : Fragment() {
    ...
    private fun showLongPressMenu(item: MetaItem) {
        ...
        // 7. Filter - NEW MENU ITEM
        // Add a filter action that opens the filter dialog
        val actionFilter = view.findViewById<View>(R.id.actionFilter)
        if (actionFilter != null) {
            actionFilter.visibility = View.VISIBLE
    ...
    private fun showFilterDialog() {
        val currentConfig = libraryViewModel.filterConfig.value ?: LibraryFilterConfig()
        val availableGenres = libraryViewModel.availableGenres.value ?: emptyList()
        ...
        LibraryFilterDialog(
            ...
            onApply = { newConfig ->
                libraryViewModel.updateFilterConfig(newConfig)
            },
            onClear = {
    ...
    private fun setupObservers() {
        libraryViewModel.libraryRows.observe(viewLifecycleOwner) { rows ->

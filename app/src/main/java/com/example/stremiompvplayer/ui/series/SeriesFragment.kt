...
import com.example.stremiompvplayer.viewmodels.SeriesViewModel
import com.example.stremiompvplayer.viewmodels.SeriesViewModelFactory
...
/**
 * Fragment for the Series page.
 * Displays series rows based on the configuration from Settings.
...
class SeriesFragment : Fragment() {
    ...
    enum class DrillDownLevel { ROWS, SERIES, SEASON, GENRE_RESULTS, CAST_RESULTS }
    ...
    // Adapter for drill-down content (seasons/episodes/genre results/cast results)
    private var drillDownAdapter: PosterAdapter? = null

    // Store current series rows for restoration
    private var cachedSeriesRows: List<HomeRow> = emptyList()
    ...
    private fun loadGenreSeries(genreId: Int, genreName: String) {
        viewLifecycleOwner.lifecycleScope.launch {
            binding.progressBar.visibility = View.VISIBLE
            try {
                val series = viewModel.fetchSeriesByGenre(genreId)
                showDrillDownContent(series, "$genreName Series")
            } catch (e: Exception) {
                // Handle error
            } finally {
    ...
    private fun updateHeroBanner(item: MetaItem) {
        ...
        heroUpdateJob = viewLifecycleOwner.lifecycleScope.launch {
            ...
            Glide.with(this@SeriesFragment)
    ...
    private fun setupObservers() {
        viewModel.seriesRows.observe(viewLifecycleOwner) { rows ->
            cachedSeriesRows = rows
    ...
    /**
     * Shows drill-down content (seasons, episodes, genre results, or cast results) in place of series rows
     */
    ...
    /**
     * Restores the series rows view from drill-down state
     */
    private fun restoreSeriesRowsView() {
        ...
        if (cachedSeriesRows.isNotEmpty()) {
            rowAdapter.updateData(cachedSeriesRows, this::showLongPressMenu)
            if (cachedSeriesRows[0].items.isNotEmpty()) {
                updateHeroBanner(cachedSeriesRows[0].items[0])
            }
        }
    ...
    fun handleKeyDown(keyCode: Int, event: android.view.KeyEvent?): Boolean {
        // No special key handling needed for Series page currently
        return false
    }
